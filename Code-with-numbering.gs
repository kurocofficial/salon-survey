// ===============================================
// ヘアサロンアンケート用 Google Apps Script (ナンバリング対応版)
// ===============================================
...
function writeStoreData(spreadsheet, data) {
  ...
  // No.列を中央揃えに設定
  storeSheet.getRange(lastRow + 1, 1).setHorizontalAlignment('center');
...
function writeStylistData(spreadsheet, data, storeRowNumber) {
  ...
  data.stylists.forEach(stylist => {
    ...
    const stylistData = [
      ...
      storeRowNumber // 店舗データとの関連付け
    ];
    
    stylistSheet.getRange(lastRow + 1, 1, 1, stylistData.length).setValues([stylistData]);
    
    // No.列と関連店舗No.列を中央揃えに設定
    stylistSheet.getRange(lastRow + 1, 1).setHorizontalAlignment('center');
    stylistSheet.getRange(lastRow + 1, 10).setHorizontalAlignment('center');
    
    console.log('スタイリストデータを追加しました (No.' + rowNumber + '):', stylistData);
  });
}
